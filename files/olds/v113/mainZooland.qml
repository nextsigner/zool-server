...
*/
    ...
    Component.onCompleted: {
        ...
        if(Qt.platform.os==='linux' || Qt.platform.os==='android'){
            let compMinyma=Qt.createComponent('./modules/comps/MinymaClient/MinymaClient.qml')
            //let objMinyma=compMinyma.createObject(app, {loginUserName: 'zooland', host: apps.minymaClientHost, port: apps.minymaClientPort})
            let objMinyma=compMinyma.createObject(app, {loginUserName: 'zooland', host: '192.168.1.51', port: 12345})
            objMinyma.newMessageForMe.connect(function(from, data) {
                if(data==='up'){
                    minymaClient.sendData(minymaClient.loginUserName, from, 'Zooland responde.')
                }
